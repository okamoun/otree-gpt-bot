...
class GPTSoup(BeautifulSoup):
    ...
    def clean_form_answers(answer_dict,fields_list):
        ...
        for f in fields_list:
            logging.info(f'cleaning answer for answer {answer_dict} for field {f}')
            if 'choices' in f and f['f_id'] in answer_dict:
                answer=GPTSoup.match_answer_to_choices(answer_dict[f['f_id']],f['choices'])
                if answer is  None:
                    new_answer_dict.pop(f['f_id'])
                else :
                    new_answer_dict[f['f_id']]=answer[0]
        return new_answer_dict
class GPTBotDialogue(otree.api.Bot):

    pages_seq=[]
    openai = QueryWithCache()
    default_engine_param={'n':5}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "default_engine_param" in self.session.config:
            self.default_engine_param.update(self.session.config['engine_param'])
        ...
        self.part_profile_dict = pd_profile.loc(get_profile_id()).to_dict()
        ...
        if self.case == 'gpt':
            for page in self.pages_seq:
                ...
                if page.__name__ != self.participant._current_page_name:
                    ...
                    continue
                dform = self.fill_html_form(page, self.html, use_cache=use_cache,use_profile=self.use_profile)
    ...
    def prompt_for_radio(self,f):
        text = f['text']
        choices = f['choices']
        logging.info(f'prompt radio for field {f} ')
        ...
        for c in results:
            ...
            try:
                ...
                if clean_dform_len == nb_fields:
                    ...
                    break
                elif len(clean_dform[list(clean_dform.keys())[0]])==nb_fields:
                    ...
                    best_form_field = {tf[i]['f_id']: clean_dform[list(clean_dform.keys())[i]] for i in range(nb_fields)}
                    logging.info(f'multiple values return {best_form_clean_len} using {dform_txt} nb field {nb_fields}')
    ...
    def fill_html_form(self, page, html, use_cache=True,engine_param={},remove=['debug'],use_profile=True,use_participant_memory='PAQ' ):
        ...
        for f in field_list :
            text = f['text']
            pf = self.prompt_ft.get(f['field_type'],self.prompt_ft['default'])
            prompt =pf(self,f)
            logging.info(f'prompt for field {f} : {prompt} ' )
            ...
            if 'P' in use_participant_memory:
                ...
                if os.path.exists(memory_file):
                    ...
                    if use_profile:
                        ...
                        part_profile_dict = self.part_profile_dict
